...
setInterval(function () {
  ...
  lastUpdate = now; // PLAYER MOVEMENT

  Player.friction();
  checkMovement();
  Player.speedLimit();
  Player.updatePosition(); // CAMERA ADJUSTING

  Camera.adjust(Player); // PHYSICS

  ambientObjects.forEach(function (e) {
    if (e.physics.solid == true & Player != e) {
      testCollisions(Player, e);
    }
  });
  animationUpdate(); // PARTICLE HANDLING

  particleUpdate(); // ANIMATION HANDLING

  animation(ambientObjects); // RENDERING
  ...
  if (Object.entries(otherPlayers).length != 0) {
    ...
    Object.entries(otherPlayers).forEach(function (p) {
      render([p[1]]);
    }); // render(otherPlayers, true);
  } // INTERFACE


  checkAbilities(); // CURSOR

  updateCursor(c);
  updateJoystick(c); // NETWORK
...
//# sourceMappingURL=/main.9276962c.js.map